...
This Java "Autograding Model" library automatically evaluates projects based on a configurable set of metrics. Currently, the following metrics are available:
...
The full list of available metrics can be found in the documentation of the
https://github.com/jenkinsci/analysis-model[analysis-model] and https://github.com/jenkinsci/coverage-model[coverage-model] libraries, that are used by the autograding library to parse the results of the individual tools.
...
When you use this library to compute an autograding score, you must define the impact on the overall score and the individual scoring criteria for each metric using a JSON configuration, as shown in the following example.
Details about each metric can be found in the documentation at the end of this document.
...
When you use this library to generate a metrics report without scores, you must define the individual metrics and their configuration in a JSON file, as shown below.
